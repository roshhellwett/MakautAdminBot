...
async def start_group_bot():
    ...
    app = ApplicationBuilder().token(token).read_timeout(30).connect_timeout(30).build() [cite: 58]
    ...
    async def group_monitor_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if violation:
            try:
                ...
                hit_limit = await add_strike(user.id) [cite: 67]
                ...
                if hit_limit:
                    ...
                    # NEW: Auto-delete Admin Security reports after 1 hour
                ...
                await warn_msg.delete() [cite: 73]
            except Exception as e:
                logger.error(f"Enforcement Error: {e}")

    app.add_handler(MessageHandler(filters.StatusUpdate.NEW_CHAT_MEMBERS, welcome_handler)) [cite: 73]
